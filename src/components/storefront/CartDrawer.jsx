      ...
      >
        ...
        <div className="flex items-center justify-between border-b border-slate-100 px-5 py-4 dark:border-slate-700">
          ...
          <button
            type="button"
            onClick={onClose}
            className="flex h-8 w-8 items-center justify-center rounded-full text-slate-400 hover:bg-slate-100 hover:text-slate-600 transition dark:text-slate-500 dark:hover:bg-slate-800 dark:hover:text-slate-300"
          >
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4">
          {cart.length === 0 ? (
            <div className="flex flex-col items-center justify-center pt-16 text-center">
              <svg className="mb-4 h-16 w-16 text-slate-200 dark:text-slate-700" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <button
                type="button"
                onClick={onClose}
                className="mt-4 text-sm font-medium text-[#2f0f6b] hover:underline dark:text-[#a78bfa]"
              >
                Continue shopping
              </button>
            </div>
              ...
              {cart.map((item, i) => (
                <li key={`${item.productSlug}-${item.variantId || ''}`} className="flex gap-3">
                  ...
                  <div className="flex flex-1 flex-col justify-between gap-1 min-w-0">
                    <div>
                      <p className="text-sm font-medium text-slate-900 line-clamp-1 dark:text-slate-100">{item.title}</p>
                      {item.variantName && item.variantName !== 'Default' && (
                        <p className="text-xs text-slate-400 dark:text-slate-500">{item.variantName}</p>
                      )}
                    </div>
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="min-w-[1.5rem] text-center text-sm font-medium text-slate-900 dark:text-slate-100">{item.quantity}</span>
                        <button
                          type="button"
                          onClick={() => handleQty(i, 1)}
                          className="flex h-7 w-7 items-center justify-center rounded-md border border-slate-200 text-sm text-slate-600 hover:bg-slate-50 transition dark:border-slate-600 dark:text-slate-300 dark:hover:bg-slate-700"
                      ...
                      <div className="flex items-center gap-2">
                        <span className="text-sm font-semibold text-[#2f0f6b] dark:text-[#a78bfa]">৳{(Number(item.price) * item.quantity).toLocaleString()}</span>
                        <button
                          type="button"
                          onClick={() => handleRemove(i)}
                          className="text-slate-300 hover:text-red-500 transition dark:text-slate-600 dark:hover:text-red-400"
                        >
