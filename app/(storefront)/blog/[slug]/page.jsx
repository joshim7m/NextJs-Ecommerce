...
export async function generateMetadata({ params }) {
  ...
  return {
    title: `${post.title} | Cabinet Closet Blog`,
    description,
...
async function getPost(slug) {
  ...
  const readingTime = Math.max(1, Math.ceil((post.content.replace(/<[^>]+>/g, '').length / 1000) * 2));
...
export default async function BlogPostPage({ params }) {
  ...
  const { post, related, ads, contentWithAds, readingTime } = data;
  ...
  const jsonLd = {
    ...
    author: post.category?.authorName ? {
      '@type': 'Person', name: post.category.authorName,
    } : undefined,
    publisher: { '@type': 'Organization', name: 'Cabinet Closet' },
    mainEntityOfPage: {
      '@type': 'WebPage',
      '@id': `${process.env.NEXT_PUBLIC_SITE_URL || 'https://cabinetcloset.com'}/blog/${post.slug}`,
    },
  };
    ...
    <>
      ...
      <article>
        ...
        <div className="border-b border-slate-100 bg-white/80 backdrop-blur-sm dark:border-slate-700 dark:bg-slate-900/80">
          <div className="mx-auto flex max-w-4xl items-center gap-2 px-4 py-3 text-xs text-slate-400 sm:px-6">
            <Link href="/" className="hover:text-[#2f0f6b] dark:hover:text-[#a78bfa] transition-colors">Home</Link>
            <svg className="h-3 w-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" /></svg>
            <Link href="/blog" className="hover:text-[#2f0f6b] dark:hover:text-[#a78bfa] transition-colors">Blog</Link>
        ...
        <div className="mx-auto max-w-4xl px-4 pb-12 sm:px-6 sm:pb-16">
          ...
          <div className="mt-10 flex items-center justify-between border-t border-slate-100 dark:border-slate-700 pt-6">
            <Link href="/blog" className="inline-flex items-center gap-1.5 text-sm font-medium text-slate-600 dark:text-slate-400 hover:text-[#2f0f6b] dark:hover:text-[#a78bfa] transition">
